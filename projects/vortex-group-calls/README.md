...
| Script file | VortexGroupCalls.gs (in this repo at projects/vortex-group-calls/VortexGroupCalls.gs) |
...
All conversions use Utilities.formatDate with IANA timezone IDs -- never manual offsets. Abbreviations (BST, PDT, CEST etc.) computed live at send time so DST changes automatically.
...
To get the weekly email, join our Google Group:
...
Apple Calendar / Outlook / anything else -- add a calendar by URL:
